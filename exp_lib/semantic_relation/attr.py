...
def worker(sim_threshold, query_nodes, topk, num_intruders, num_buckets, output_folder):
    ...
    for q in query_nodes:
        coherence_tests = attr(sim_threshold, q, topk, num_intruders, num_buckets)
...
def on_completion(result):
    ...
    pickle.dump(result[1], open(f"{output_folder}/tmp/attr_{result[1][0][0]}.pkl", "wb"))
    logger.info(f"Node {result[1][0][0]} group completed")
    del result


def attr(sim_threshold, query_node, topk, num_intruders, num_buckets):
...
def attr_coherence_test(
    ...
    node_attr = data.x
    sim_mx = cosine_similarity(node_attr, node_attr)
    ...
    if len(share_data)==0:
        ...
        share_data[0] = sim_mx
    pickle.dump(sim_mx, open(f"{output_folder}/attr.pkl", "wb"), protocol=4)
